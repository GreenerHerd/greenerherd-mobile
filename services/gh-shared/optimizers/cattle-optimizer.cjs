...
function optimizeFeedMix(requirements, feeds, options = {}) {
  ...
  for (const feed of normalizedFeeds) {
    ...
    // Add intake bounds (MaxIntake and optional MinIntake) as variable bounds.
    // We treat MinIntake as a soft "preference" enforced by an LP lower-bound.
    if ((feed.maxIntakePercentOfDM && feed.maxIntakePercentOfDM > 0) ||
      ...
      // Calculate bounds as kg as-fed for this decision variable.
      ...
      // Check MinIntake constraints (prefer larger/min-more-restrictive lower bounds)
      if (feed.minIntakePercentOfDM > 0 && requirements.dryMatterKg > 0) {
        const minFromPercentDM =
          (feed.minIntakePercentOfDM / 100) * requirements.dryMatterKg / feed.dmKgPerKg;
      ...
      if (feed.minIntakeWeightInKgPerAnimal > 0) {
        const minFromWeightPerAnimal = feed.minIntakeWeightInKgPerAnimal * numAnimals;
      ...
      if (minIntakeKg > 0) {
        // Ensure min does not exceed max if both are present.
        ...
        // Keep default LP lower-bound at 0.
        v.min = 0;
      }
      if (maxIntakeKg < Infinity) {
...
function normalizeFeed(feed, index) {
  ...
  return {
    ...
    type: feed.type || feed.feedType || null,
  };
}
